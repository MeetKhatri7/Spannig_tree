...
def take_input(G):
    def create_root_2(no_of_nodes, no_of_edges):
        def submit_2():
            for no_edges in range(no_of_edges):
                edge = eval(edge_entry_list[no_edges].get())
                weight = int(weight_entry_list[no_edges].get())
    ...
    def submit_1():
        no_of_nodes = int(node_entry.get())
        no_of_edges = int(edge_entry.get())
        print(f"No. of nodes = {no_of_nodes}")
        print(f"No. of edges = {no_of_edges}")
        root_1.destroy()
        create_root_2(no_of_nodes, no_of_edges)
